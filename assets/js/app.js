function makeResponsive() {
    ...
    function xScale(stateData, chosenXaxis) {
        var xlinear = d3.scaleLinear()
                        .domain([d3.min(stateData, d => d[chosenXaxis]) - ((d3.max(stateData, d => d[chosenXaxis] - d3.min(stateData, d => d[chosenXaxis]))/50)),
                    d3.max(stateData, d => d[chosenXaxis]) + ((d3.max(stateData, d => d[chosenXaxis] - d3.min(stateData, d => d[chosenXaxis]))/50))])
                    .range([0, width]);
        return xlinear;
    };
    ...
    function yScale(stateData, chosenYaxis) {
        var ylinear = d3.scaleLinear()
                        .domain([d3.min(stateData, d => d[chosenYaxis]) - ((d3.max(stateData, d => d[chosenYaxis] - d3.min(stateData, d => d[chosenYaxis]))/10)), 
                        d3.max(stateData, d=> d[chosenYaxis]) + ((d3.max(stateData, d => d[chosenYaxis] - d3.min(stateData, d => d[chosenYaxis]))/10))])
                        .range([height,0]);
        return ylinear;
    };
